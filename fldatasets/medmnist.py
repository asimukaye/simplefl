...
import medmnist
import numpy as np
from config import DatasetModelSpec

logger = logging.getLogger(__name__)

class MedClfDataset(Subset): 
...
def fetch_medmnist_dataset(dataset_name:str, root:str ):
    ...
    raw_train = MedClfDataset(raw_train, dataset_name.upper(), 'CLIENT')

    raw_test = DataClass(root=root, split='test', transform=transforms[1], download=True)
    raw_test = MedClfDataset(raw_test, dataset_name.upper(), 'SERVER')
    # ic(len(raw_test))
