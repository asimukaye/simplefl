...
def sample_with_mask(
    mask, ideal_samples_counts, concentration, num_classes, need_adjustment=False
):
    ...
    selection_prob_raw = np.random.dirichlet(
        alpha=np.ones(num_remaining_classes) * concentration, size=1
    ).squeeze()
    selection_prob = mask.copy()
    selection_prob[selection_prob == 1.0] = selection_prob_raw
    selection_prob /= selection_prob.sum()

    # calculate per-class sample counts based on selection probabilities
    if (
        need_adjustment
    ):  # if remaining samples are not enough, force adjusting sample sizes...
        selected_counts = (
            selection_prob
            * ideal_samples_counts
            * np.random.uniform(low=0.0, high=1.0, size=len(selection_prob))
        ).astype(int)
    else:
...
def get_dirichlet_split_2(
    dataset: Dataset, num_splits, num_classes, cncntrtn
) -> dict[int, np.ndarray]:
    ...
    class_samples_counts = dict(
        zip(
            [i for i in range(num_classes)],
            [len(class_idx) for class_idx in class_indices],
        )
    )
    ...
    for k in log_tqdm(
        ...
        remaining_mask = np.where(
            np.array(list(class_samples_counts.values())) > 0, 1.0, 0.0
        )
        selected_counts = sample_with_mask(
            remaining_mask, ideal_samples_counts, cncntrtn, num_classes
        )

        # check if enough samples exist per selected class
        expected_counts = np.subtract(
            np.array(list(class_samples_counts.values())), selected_counts
        )
        valid_mask = np.where(expected_counts < 0, 1.0, 0.0)
        ...
        while sum(valid_mask) > 0:
            ...
            adjusted_mask = (
                remaining_mask.astype(bool) & (~valid_mask.astype(bool))
            ).astype(float)
...
def dirichlet_noniid_split_fixed(
    dataset: Subset,
    n_clients: int,
    alpha: float,
) -> dict[int, np.ndarray]:
    ...
    for c_idcs, fracs in zip(class_idcs, label_distribution):
        ...
        for neediest_client, idx in zip(client_order, idx_order):
            client_idcs[neediest_client] = np.append(
                client_idcs[neediest_client], idcs[idx]
            )
