...
def pathological_non_iid_split(
    dataset: Subset, n_clients: int, n_classes_per_client: int
) -> dict[int, np.ndarray]:
    ...
    clients_idcs = {
        k: np.array(idcs) for k, idcs in zip(range(n_clients), clients_idcs)
    }
    return clients_idcs
...
def get_patho_split(
    dataset: Subset, num_splits: int, num_classes: int, mincls: int
) -> dict[int, np.ndarray]:
    try:
        assert mincls >= 2
    except AssertionError as e:
        logger.exception(
            "[DATA_SPLIT] Each client should have samples from at least 2 distinct classes!"
        )
        raise e

    # get unique class labels and their count
    inferred_classes, unique_inverse, unique_count = np.unique(
    ...
    assert (
        len(inferred_classes) == num_classes
    ), "Inferred classes do not match the expected number of classes"
    ...
    split_indices = [
        np.array_split(np.random.permutation(indices), num_shards_per_class)
    ...
    class_shards_counts = dict(
        zip(
            ...
            [len(split_idx) for split_idx in split_indices],
        )
    )
    ...
    for _ in range(num_classes):
        ...
        selection_prob = np.where(
            np.array(list(class_shards_counts.values())) > 0, 1.0, 0.0
        )
        ...
        try:
            selected_classes = np.random.choice(
                num_classes, mincls, replace=False, p=selection_prob
            )
            ...
            selected_classes = np.random.choice(
                num_classes, mincls, replace=True, p=selection_prob
            )

        # assign shards in randomly selected classes to current client
        for it, class_idx in enumerate(selected_classes):
            selected_shard_indices = np.random.choice(len(split_indices[class_idx]), 1)[
                0
            ]
